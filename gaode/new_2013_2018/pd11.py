...
def new_process(chunk):
    ...
    try:
        ...
        csv_file_name = './{}-{}.csv'.format(int(time.time()), os.getpid())
        # time.sleep(3)
        chunk.to_csv(os.path.join(config.TARGET_PATH, csv_file_name), index=False, mode='a+')
        print('[success]-{}-generate-{}'.format(os.getpid(), csv_file_name))
    except Exception as e:
        print('[error]-{}-{}'.format(os.getpid(), e))
        csv_file_name = './error-{}-{}.csv'.format(int(time.time()), os.getpid())
        chunk_copy.to_csv(os.path.join(config.ERROR_FILE_PATH, csv_file_name), index=False, mode='a+')
...
def start(file_name):
    ...
    if not os.path.exists(config.ERROR_FILE_PATH):
        os.mkdir(config.ERROR_FILE_PATH)
...
if __name__ == '__main__':
    start(config.SOURECE_FILE_PATH)
