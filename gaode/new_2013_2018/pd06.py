...
def package_data(addr):
    try:
        ...
        print('[error]-request no respone')
        return "", "", ""
...
def new_process(chunk):
    ...
    print('[success]-gen {}'.format(csv_file_name))
...
def start(file_name):
    ...
    group = (chunk for chunk in pd.read_csv(file_name, chunksize=1000))
