...
logging.basicConfig(filename='example8.log', level = logging.DEBUG, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def go():
    ...
    for page in range(1, pages + 1):
        print('[这是第{}页]\n'.format(page))
        url = 'http://www.allitebooks.org/page/{}/?s=sql'.format(page)
        ...
        for item3 in res:
            ...
            for item2 in res2:
                ...
                with open('./books_list_2.txt','a')as f:
                    f.write(item2+'\n')
                print('[write]-have write {} to txt'.format(item2))
