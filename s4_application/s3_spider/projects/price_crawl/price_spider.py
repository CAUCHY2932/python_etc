...
from . import spider_class as spder
from . import setting

__author__ = 'young'


def main(pattern_mode):
    crawl_spider = spder.Crawl_price()
    for item in setting['SITE_LIST']:
        html_text = crawl_spider.get_html_source_code(item)
        if html_text is None:
            continue  # 出口一：
        order_num = crawl_spider.select_mode(link_list=setting['LINK_LIST'], site=item)
        if order_num == -1:
            str_taget = ''
        else:
            obj = crawl_spider.htmlish(html_text)
            str_target = crawl_spider.regex_parse(pattern_mode,text=obj)
