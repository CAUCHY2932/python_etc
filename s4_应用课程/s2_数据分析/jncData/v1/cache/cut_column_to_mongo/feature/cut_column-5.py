...
class Cutter(object):
    ...
    def cut_to_record(self, file_name, encoding='utf-8'):
        self.__cut_rows(file_name)
        self.__cut_feature(self.feature_str)
        self.__data_model()
    ...
    def __cut_rows(self, file_name, encoding='utf-8'):
        ...
        # after_cut = line.split(',')
        # self.record_id = after_cut[0]
        # self.length = after_cut[1]
        # self.feature_list = after_cut[2]

    def __cut_feature(self, cut_str):
        soh = chr(int('0x01', 16))
        stx = chr(int('0x02', 16))
        etx = chr(int('0x03', 16))
        ...
        for i in after_cut_str_list:
            ...
            self.feature_list = yield {'feature_field_id': single_feature[0],
                                       'feature_id': single_feature[1],
                                       'feature_value': single_feature[2]}

    def __data_model(self):
        self.data_list = {'_id': self.record_id,
                          'length': self.length,
