...
logging.basicConfig(filename='example7.log', level = logging.DEBUG, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def go():
    ...
    for page in range(1, pages + 1):
        ...
        for item3 in res:
            ...
            for item2 in res2:
                print('link is {}'.format(item2))
                with open('./books_list.txt','a')as f:
                    f.write(item2+'\n')
                print('have write {} to txt'.format(item2))
